export const PLAYER_MIN_ATTACK = 20;
export const PLAYER_MAX_ATTACK = 100;

export const WARRIOR_ADD_HEALTH_PER_LEVEL = 75;
export const WARRIOR_ADD_MANA_PER_LEVEL = 75;

export const MAGE_ADD_HEALTH_PER_LEVEL = 25;
export const MAGE_ADD_MANA_PER_LEVEL = 75;

export const ARCHER_ADD_HEALTH_PER_LEVEL = 50;
export const ARCHER_ADD_MANA_PER_LEVEL = 50;

export const HEALTH_POTION_PRICE = 50;
export const MANA_POTION_PRICE = 50;

export const ORC_HEALTH_POINTS = 100;
export const ORC_MIN_ATTACK = 15;
export const ORC_MAX_ATTACK = 100;
export const ORC_EXPERIENCE_FOR_KILL = 35;
export const ORC_MIN_LOOT = 70;
export const ORC_MAX_LOOT = 50;

export const DRAGON_HEALTH_POINTS = 250;
export const DRAGON_MIN_ATTACK = 15;
export const DRAGON_MAX_ATTACK = 100;
export const DRAGON_EXPERIENCE_FOR_KILL = 35;
export const DRAGON_MIN_LOOT = 70;
export const DRAGON_MAX_LOOT = 50;

export const FERUMBRAS_HEALTH_POINTS = 500;
export const FERUMBRAS_MIN_ATTACK = 15;
export const FERUMBRAS_MAX_ATTACK = 100;
export const FERUMBRAS_EXPERIENCE_FOR_KILL = 35;
export const FERUMBRAS_MIN_LOOT = 200;
export const FERUMBRAS_MAX_LOOT = 500;
